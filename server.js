...
function send(res, status, body, contentType = 'application/json') {
  ...
  res.writeHead(status, {
    ...
    'Access-Control-Allow-Headers': 'Content-Type,X-Device-Id,X-File-Category',
  });
  res.end(data);
}
      ...
      while (true) {
        if (!parsingFile) {
          ...
          if (filenameMatch) {
            ...
            const deviceId = fields['deviceId'] || req.headers['x-device-id'] || 'unknown';
            const category = fields['category'] || req.headers['x-file-category'] || 'file';
...
<script>
  ...
  function renderCard(f) {
    ...
    const iconMap = {image:'🖼',video:'🎬',download:'⬇',file:'📄'};
    const thumb = isImg
    ...
    const tagLabel = {image:'Ảnh',video:'Video',download:'Download',file:'Tài liệu'}[f.category]||f.category;
