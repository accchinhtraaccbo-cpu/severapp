...
function startSelfPing() {
  ...
  // Ưu tiên dùng RENDER_EXTERNAL_URL nếu có (Render tự set),
  // rồi đến APP_URL do người dùng cấu hình, cuối cùng dùng localhost
  ...
  setInterval(() => {
    ...
    http.get(target, (res) => {
      ...
      console.warn(`[PING] Lỗi ping: ${err.message}`);
    });
  }, PING_INTERVAL);

  console.log(`[PING] Self-ping mỗi 5 phút → ${selfUrl}/api/ping`);
}
...
<script>
  ...
  function renderMain() {
    ...
    panel.innerHTML = \`
      ...
      <div class="tab-bar">
        \${['all','user','game','sys'].map(t => \`
          <button class="tab \${activeTab===t?'active':''}" onclick="activeTab='\${t}';renderMain()">
            \${{all:'📱 Tất cả',user:'👤 User',game:'🎮 Game',sys:'⚙️ Hệ thống'}[t]}
          </button>
        \`).join('')}
      </div>
