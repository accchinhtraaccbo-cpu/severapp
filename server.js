...
route('POST', /^\/api\/devices\/(?<deviceId>[^/?]+)\/command/, async (req, res) => {
  ...
  queue.push({ cmdId, action, packageName });
...
<script>
  ...
  // ── Commands ────────────────────────────────────
  async function sendCommand(deviceId, action, packageName) {
    pendingPkgs.add(packageName);
    renderMain();
    try {
      await api('POST', \`/api/devices/\${deviceId}/command\`, { action, packageName });
      // Optimistic UI
