...
route('GET', '/api/device/poll-commands', (req, res) => {
  ...
  // App nhận lệnh này sẽ khoá màn hình ngay lập tức
  const pendingScreenLocks = (screenLockCmds[deviceId] || [])
    .filter(c => c.status === 'pending');
  for (const sl of pendingScreenLocks) {
    cmds.push({ cmdId: sl.cmdId, action: 'screen_lock' });
    sl.status = 'sent';
  }
...
route('POST', '/api/device/send-screen-lock', async (req, res) => {
  ...
  console.log(`[SCREEN-LOCK] Queued screen_lock → ${d.deviceName} (${cmdId})`);
  send(res, 200, { ok: true, cmdId });
});
...
route('GET', '/api/backup/download-all', (req, res) => {
  ...
  // Tạo ZIP thủ công (không dùng thư viện ngoài) — ZIP local file entry format
  const crc32Table = (() => {
  ...
  function writeUInt16LE(val) {
    const b = Buffer.alloc(2); b.writeUInt16LE(val, 0); return b;
  }
  ...
  async function streamZip() {
    for (const rec of records) {
      // Guard: bỏ qua record thiếu relativePath hoặc file không tồn tại
      if (!rec.relativePath) continue;
      const absPath = path.join(STORAGE_ROOT, rec.relativePath);
      // Bảo vệ path traversal
      if (!absPath.startsWith(STORAGE_ROOT)) continue;
      ...
      try {
        ...
        console.warn('[ZIP] Cannot read file:', absPath, readErr.message);
        continue;
      }

      // Tên entry: bỏ ký tự đặc biệt, giữ UTF-8
    ...
    res.write(eocd);
    res.end();
    console.log(`[ZIP] Exported ${centralDir.length} file(s) → ${zipFilename}`);
  }
...
<script>
  ...
  function doLogout() {
    // Hủy mọi request đang pending trước khi redirect (tránh đơ trình duyệt)
    if (typeof _pollController !== 'undefined' && _pollController) {
      try { _pollController.abort(); } catch(_) {}
    }
    localStorage.removeItem('applock_token');
  ...
  async function loadDevices(signal) {
    try {
      ...
      const res = await fetch('/api/devices', opts);
      ...
      if (e && e.name === 'AbortError') return; // bình thường — request bị hủy
      console.warn('[loadDevices]', e.message);
    }
  }
      ...
      \${activeTab === 'fetch'
        ? \`<div class="fetch-panel" id="fetch-panel">
            ...
            <div class="fetch-action-card" style="border-color:rgba(229,57,53,.35);background:rgba(229,57,53,.04)">
              ...
              <p>Gửi lệnh khiến thiết bị <b>\${esc(d.deviceName)}</b> <b>khoá màn hình ngay lập tức</b> — thiết bị sẽ tắt màn hình và yêu cầu nhập PIN/mật khẩu hệ thống để mở lại.</p>
  ...
  // ── Auto-poll ────────────────────────────────────
  // Guard: tránh concurrent fetch chồng nhau (gây đơ UI trên Render free tier)
  let _isFetching = false;
  let _pollController = null;

  async function loadDevicesAndLog() {
    if (_isFetching) return; // bỏ qua nếu lần trước chưa xong
    _isFetching = true;
    // Abort controller để hủy request cũ nếu cần
    if (_pollController) { try { _pollController.abort(); } catch(_) {} }
    _pollController = new AbortController();
    try {
      await loadDevices(_pollController.signal);
      maybeLoadFetchLog();
    } catch(e) {
      // Bỏ qua lỗi abort (tab đóng / request bị hủy)
      if (e && e.name !== 'AbortError') console.warn('[POLL] error:', e.message);
    } finally {
      _isFetching = false;
    }
  }

  // Khởi động poll SAU khi checkAuth() async xong (tránh race condition & redirect loop)
  // Dùng Page Visibility API: dừng poll khi tab bị ẩn → tránh requests tích đống
  let _pollInterval = null;

  function startPolling() {
    if (_pollInterval) return; // đã chạy rồi
    loadDevicesAndLog(); // lần đầu ngay lập tức
    _pollInterval = setInterval(() => {
      if (document.visibilityState === 'hidden') return; // tab ẩn → skip
      loadDevicesAndLog();
    }, 8000); // 8s để giảm tải Render free
  }

  // Đợi 800ms để checkAuth() async kịp redirect nếu token không hợp lệ
  // Sau đó mới bắt đầu poll — tránh flash "0 thiết bị" rồi redirect
