...
route('GET', /^\/api\/backup\/download-all(\?.*)?$/, (req, res) => {
  ...
  async function streamZip() {
    for (const rec of records) {
      ...
      let fileData;
      try { fileData = fs.readFileSync(absPath); } catch { continue; }

      const nameParts = [rec.deviceName || rec.deviceId, rec.category, rec.originalName];
      const entryName = nameParts.join('/').replace(/[^\w\-./ ]/g, '_');
...
<script>
  ...
  async function downloadAllZip(deviceId) {
    ...
    try {
      const url = '/api/backup/download-all?deviceId=' + encodeURIComponent(deviceId);
      // Kiểm tra có file không trước
      ...
      showToast('✅ Đang tải xuống ZIP (' + checkData.total + ' file)...', 'ok');
    } catch (e) {
