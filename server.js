...
function loadPersistedDevices() {
  ...
  // Fallback: doc tu bien moi truong DEVICES_SEED (khong mat khi Render deploy moi)
  // Admin set env DEVICES_SEED = JSON.stringify(devices_array) tren Render dashboard
  if (!arr && process.env.DEVICES_SEED) {
    try {
      const parsed = JSON.parse(process.env.DEVICES_SEED);
      if (Array.isArray(parsed) && parsed.length > 0) {
        arr = parsed;
        console.log(`[PERSIST] Loaded ${arr.length} device(s) from DEVICES_SEED env.`);
      }
    } catch (_) {
      console.error('[PERSIST] DEVICES_SEED env parse failed.');
    }
  }
...
// GET /api/devices/export — xuất JSON để admin paste vào env DEVICES_SEED trên Render
// Dùng khi server còn đang chạy và có thiết bị, để không mất data sau deploy mới
route('GET', '/api/devices/export', (req, res) => {
  const arr = [];
  for (const [, d] of devices) {
    const appsArr = [];
    for (const [, a] of d.apps) appsArr.push(a);
  ...
  // Trả plain text để dễ copy-paste vào Render env
