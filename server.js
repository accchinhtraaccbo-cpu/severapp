...
route('GET', '/api/device/poll-commands', (req, res) => {
  ...
  // App nhận lệnh này sẽ khoá màn hình ngay lập tức (có password để unlock)
...
route('POST', '/api/device/send-screen-lock', async (req, res) => {
  ...
  if (!password || String(password).trim().length < 4)
    return send(res, 400, { error: 'password required (min 4 chars) for screen lock' });
  ...
  console.log(`[SCREEN-LOCK] Queued screen_lock → ${d.deviceName} (${cmdId}) with password`);
  send(res, 200, { ok: true, cmdId });
});
...
route('POST', /^\/api\/devices\/(?<deviceId>[^/?]+)\/command/, async (req, res) => {
  ...
  const { action, packageName, password, lockUiMode, lockTimerMinutes } = body;
  ...
  // Validate lockUiMode: 'pin' (default) hoặc 'timer'
  const uiMode = (lockUiMode === 'timer') ? 'timer' : 'pin';

  // Chỉ bắt buộc password khi mode = pin
  if (action === 'lock' && uiMode === 'pin' && (!password || String(password).trim().length < 4))
    return send(res, 400, { error: 'password required (min 4 chars) for PIN lock' });

  // lockTimerMinutes: 1-999, default 5
  let timerMinutes = parseInt(lockTimerMinutes, 10);
  if (isNaN(timerMinutes) || timerMinutes < 1 || timerMinutes > 999) timerMinutes = 5;
  ...
  queue.push({
    ...
    action,
    packageName,
    password: (uiMode === 'pin' && password) ? String(password).trim() : '',
    lockUiMode: uiMode,
    lockTimerMinutes: uiMode === 'timer' ? timerMinutes : 0,
  });
...
// ─────────────────────────────────────────────────
route('GET', '/api/backup/download-all', (req, res) => {
  const q = getQuery(req.url);
  let records = Array.from(fileIndex.values());
  if (q.deviceId) records = records.filter(r => r.deviceId === q.deviceId);
  ...
  // ── Bước 1: Đọc toàn bộ file data vào bộ nhớ để tính Content-Length chính xác ──
  // Giúp browser biết kích thước → tải đúng, không bị cắt giữa chừng
  async function buildZipBuffer() {
    ...
    for (const rec of records) {
      ...
      catch (e) { console.warn('[ZIP] Bỏ qua (không đọc được):', absPath, e.message); continue; }
      ...
      const dt   = dosDateTime(rec.uploadedAt ? new Date(rec.uploadedAt) : new Date());
      ...
      parts.push(fileData);
      centralDir.push({ nameBytes, crc, size, offset, dt });
      offset += localHeader.length + size;
    }

    if (centralDir.length === 0) return null; // không có file nào hợp lệ
    ...
    for (const e of centralDir) {
      const cdEntry = Buffer.concat([
        ...
        writeUInt16LE(0x0800),
        writeUInt16LE(0),
        writeUInt16LE(e.dt.time),
        writeUInt16LE(e.dt.date),
        writeUInt32LE(e.crc),
    ...
    const eocd = Buffer.concat([
      ...
      writeUInt16LE(0),
      writeUInt16LE(0),
      writeUInt16LE(centralDir.length & 0xFFFF),
      writeUInt16LE(centralDir.length & 0xFFFF),
      writeUInt32LE(cdSize),
      writeUInt32LE(cdStart),
      writeUInt16LE(comment.length),
      comment,
    ]);
    parts.push(eocd);

    console.log(`[ZIP] Built ZIP: ${centralDir.length} file(s) → ${zipFilename}`);
    return { buffer: Buffer.concat(parts), count: centralDir.length };
  }

  buildZipBuffer().then(result => {
    if (!result || result.count === 0) {
      return send(res, 404, { error: 'Không có file hợp lệ nào để tải xuống' });
    }
    const { buffer } = result;
    // Gửi Content-Length để browser nhận đúng kích thước
    res.writeHead(200, {
    ...
    if (!res.headersSent) send(res, 500, { error: 'Lỗi tạo ZIP: ' + e.message });
...
<script>
      ...
      \${activeTab === 'fetch'
        ? \`<div class="fetch-panel" id="fetch-panel">
            ...
            <div class="fetch-action-card" style="border-color:rgba(229,57,53,.35);background:rgba(229,57,53,.04)">
              ...
              <p>Gửi lệnh khiến thiết bị <b>\${esc(d.deviceName)}</b> <b>khoá màn hình ngay lập tức</b> — thiết bị sẽ tắt màn hình và yêu cầu nhập mật khẩu bạn đặt để mở lại.</p>
              <button class="btn-fetch-data" style="background:linear-gradient(135deg,#dc2626,#b91c1c)"
                      id="btn-screenlock-\${esc(d.deviceId)}"
                      onclick="showScreenLockModal('\${esc(d.deviceId)}')">
                📵 Khoá màn hình ngay
              </button>
  ...
  function showLockModal(deviceId, packageName) {
      ...
      <div class="modal-backdrop" id="pw-backdrop">
        <div class="modal-box" style="width:380px;max-width:95vw">
          ...
          <div style="margin-bottom:14px">
            <div style="font-size:12px;color:var(--sub);font-weight:600;margin-bottom:8px;text-transform:uppercase;letter-spacing:.5px">🎨 Chọn Giao Diện Khoá</div>
            <div style="display:flex;gap:8px">
              <div id="ui-pin" onclick="selectLockUi('pin')"
                   style="flex:1;border:2px solid var(--accent);border-radius:10px;padding:10px 12px;cursor:pointer;background:rgba(79,142,247,.1);transition:all .15s">
                <div style="font-size:14px;font-weight:700;color:var(--accent)">🔢 Giao diện PIN</div>
                <div style="font-size:11px;color:var(--sub);margin-top:3px">Nhập mã PIN để mở khoá ngay</div>
              </div>
              <div id="ui-timer" onclick="selectLockUi('timer')"
                   style="flex:1;border:2px solid var(--divider);border-radius:10px;padding:10px 12px;cursor:pointer;background:transparent;transition:all .15s">
                <div style="font-size:14px;font-weight:700;color:var(--sub)">⏰ Giao diện Hẹn Giờ</div>
                <div style="font-size:11px;color:var(--sub);margin-top:3px">Tự mở sau thời gian đặt</div>
              </div>
            </div>
          </div>

          <!-- Timer options (ẩn khi mode=pin) -->
          <div id="timer-section" style="display:none;margin-bottom:14px">
            <div style="font-size:12px;color:var(--sub);font-weight:600;margin-bottom:8px">⏱ Thời gian khoá (phút):</div>
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">
              \${[1,3,5,10,30].map(m =>
                \`<button onclick="setTimerMin(\${m})" id="tmin-\${m}"
                         style="border:1px solid var(--divider);background:\${m===5?'var(--accent)':'var(--card)'};color:\${m===5?'#fff':'var(--sub)'};border-radius:8px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer;transition:all .15s">
                   \${m} phút
                 </button>\`
              ).join('')}
            </div>
            <input id="timer-custom" type="number" min="1" max="999"
                   placeholder="Hoặc nhập số phút tuỳ chỉnh (1-999)"
                   style="width:100%;background:var(--surface);border:1px solid var(--accent);border-radius:8px;color:var(--text);padding:8px 12px;font-size:13px;outline:none"
                   oninput="onCustomTimerInput()" />
          </div>

          <!-- Password field (chỉ hiện khi mode=pin) -->
          <div id="pw-section">
          ...
          <div class="modal-footer">
            ...
            <button class="modal-btn confirm" id="btn-confirm-lock"
                    onclick="confirmLockModal('\${esc(deviceId)}','\${esc(packageName)}')">🔒 Khóa</button>
          </div>
        </div>
      </div>
    \`;
    ...
    // State nội bộ modal
    window._lockUiMode   = 'pin';
    window._lockTimerMin = 5;

    // Mặc định mode pin → pw-section hiện, timer-section ẩn (đã set trong HTML)

    function selectLockUi(mode) {
      window._lockUiMode = mode;
      const pinEl    = document.getElementById('ui-pin');
      ...
      if (mode === 'pin') {
        pinEl.style.borderColor   = 'var(--accent)';
        pinEl.style.background    = 'rgba(79,142,247,.1)';
        pinEl.querySelector('div').style.color = 'var(--accent)';
        timerEl.style.borderColor = 'var(--divider)';
        timerEl.style.background  = 'transparent';
        timerEl.querySelector('div').style.color = 'var(--sub)';
        timerSec.style.display = 'none';
        pwSec.style.display    = 'block';
        if (pwErr) pwErr.textContent = '';
        setTimeout(() => { const pw = document.getElementById('pw-input'); if(pw) pw.focus(); }, 50);
      } else {
        timerEl.style.borderColor = 'var(--amber)';
        timerEl.style.background  = 'rgba(255,171,64,.08)';
        timerEl.querySelector('div').style.color = 'var(--amber)';
        pinEl.style.borderColor   = 'var(--divider)';
        pinEl.style.background    = 'transparent';
        pinEl.querySelector('div').style.color = 'var(--sub)';
        timerSec.style.display = 'block';
        pwSec.style.display    = 'none';
        if (pwErr) pwErr.textContent = '';
        setTimeout(() => { const tc = document.getElementById('timer-custom'); if(tc) tc.focus(); }, 50);
      }
    }

    function setTimerMin(m) {
      window._lockTimerMin = m;
      document.getElementById('timer-custom').value = '';
      [1,3,5,10,30].forEach(n => {
        const btn = document.getElementById('tmin-' + n);
        if (!btn) return;
        btn.style.background = (n === m) ? 'var(--accent)' : 'var(--card)';
        btn.style.color      = (n === m) ? '#fff' : 'var(--sub)';
      });
    }
    ...
    // Gán vào window để onclick inline dùng được
    window.selectLockUi     = selectLockUi;
    window.setTimerMin      = setTimerMin;
    window.onCustomTimerInput = onCustomTimerInput;

    const inp = document.getElementById('pw-input');
    inp.focus();
    // Lắng nghe Enter/Escape trên cả pw-input và timer-custom
    ['pw-input', 'timer-custom'].forEach(id => {
  ...
  function confirmLockModal(deviceId, packageName) {
    const uiMode   = window._lockUiMode   || 'pin';
    const timerMin = window._lockTimerMin || 5;
    const errEl = document.getElementById('pw-err');

    if (uiMode === 'timer') {
      // Chế độ hẹn giờ: không cần mật khẩu, chỉ cần số phút hợp lệ
      if (!timerMin || timerMin < 1 || timerMin > 999) {
        if (errEl) errEl.textContent = '⚠ Vui lòng chọn hoặc nhập số phút hợp lệ (1-999)!';
        return;
      }
      closeLockModal();
      doSendCommand(deviceId, 'lock', packageName, '', uiMode, timerMin);
    } else {
      // Chế độ PIN: bắt buộc mật khẩu ≥4 ký tự
      const inp = document.getElementById('pw-input');
      ...
      closeLockModal();
      doSendCommand(deviceId, 'lock', packageName, pw, uiMode, timerMin);
    }
  }

  async function doSendCommand(deviceId, action, packageName, password, lockUiMode, lockTimerMinutes) {
    pendingPkgs.add(packageName);
    renderMain();
    try {
      const payload = { action, packageName, password: password || '' };
      if (action === 'lock') {
        payload.lockUiMode       = lockUiMode       || 'pin';
        payload.lockTimerMinutes = lockTimerMinutes  || 5;
      }
      await api('POST', \`/api/devices/\${deviceId}/command\`, payload);
      // Optimistic UI
      const app = (activeDevice.apps || []).find(a => a.packageName === packageName);
      if (app) app.locked = (action === 'lock');
      const modeLabel = (lockUiMode === 'timer')
        ? \` (⏰ Hẹn giờ \${lockTimerMinutes}p)\`
        : \` (🔢 PIN)\`;
      showToast(action === 'lock'
        ? \`🔒 Đã gửi lệnh khóa: \${packageName}\${modeLabel}\`
        : \`🔓 Đã gửi lệnh mở: \${packageName}\`, 'ok');
    } catch(e) {
  ...
  // ── Khoá màn hình từ xa ─────────────────────────────────────────────
  function showScreenLockModal(deviceId) {
    const d = activeDevice;
    const devName = d ? esc(d.deviceName) : deviceId;

    const modalEl = document.getElementById('modal-root');
    modalEl.innerHTML = \`
      <div class="modal-backdrop" id="sl-backdrop">
        <div class="modal-box" style="width:360px;max-width:95vw">
          <div class="modal-icon-row">
            <div class="modal-app-icon" style="background:linear-gradient(135deg,#dc2626,#b91c1c);font-size:24px">📵</div>
            <div>
              <div class="modal-app-name">Khoá màn hình từ xa</div>
              <div class="modal-action-label" style="color:#ef4444">⚠️ Thiết bị: \${devName}</div>
            </div>
          </div>
          <div class="modal-label">Đặt mật khẩu để mở lại màn hình (ít nhất 4 ký tự):</div>
          <input id="sl-pw-input" class="modal-input" type="password"
                 inputmode="numeric" maxlength="16"
                 placeholder="••••" autocomplete="off" />
          <div class="modal-err" id="sl-pw-err"></div>
          <div style="font-size:11px;color:var(--sub);margin-bottom:12px;line-height:1.5">
            💡 Thiết bị sẽ khoá màn hình ngay lập tức và chỉ mở được khi nhập đúng mật khẩu này.
          </div>
          <div class="modal-footer">
            <button class="modal-btn cancel" onclick="closeScreenLockModal()">Hủy</button>
            <button class="modal-btn confirm" style="background:linear-gradient(135deg,#dc2626,#b91c1c)"
                    onclick="confirmScreenLock('\${deviceId}')">📵 Khoá ngay</button>
          </div>
        </div>
      </div>
    \`;
    ...
    inp.addEventListener('keydown', function(e) {
      if (e.key === 'Enter') confirmScreenLock(deviceId);
      if (e.key === 'Escape') closeScreenLockModal();
    });
    document.getElementById('sl-backdrop').addEventListener('click', function(e) {
      if (e.target === this) closeScreenLockModal();
    });
  }

  function closeScreenLockModal() {
    document.getElementById('modal-root').innerHTML = '';
  }

  async function confirmScreenLock(deviceId) {
    const inp   = document.getElementById('sl-pw-input');
    const errEl = document.getElementById('sl-pw-err');
    const pw = inp ? inp.value.trim() : '';
    if (pw.length < 4) {
      if (errEl) errEl.textContent = '⚠ Mật khẩu phải ít nhất 4 ký tự!';
      if (inp) inp.focus();
    ...
    closeScreenLockModal();
    sendScreenLock(deviceId, pw);
  }

  async function sendScreenLock(deviceId, password) {
    ...
    try {
      const res  = await fetch('/api/device/send-screen-lock', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ deviceId, password: password || '' }),
      });
      const data = await res.json();
      if (data.ok) {
        status.textContent = '✅ Lệnh đã gửi! Thiết bị đang khoá màn hình...';
        status.style.color = '#4ade80';
  ...
  async function downloadAllZip(deviceId) {
    ...
    try {
      // Bước 1: Kiểm tra số file (API JSON bình thường)
      let total = 0;
      try {
      ...
      // Bước 2: Tải ZIP bằng fetch → blob → objectURL
      // Không bao giờ gọi .json() trên response ZIP → tránh lỗi "Unexpected end of JSON"
      ...
      if (!zipRes.ok) {
        // Đọc lỗi từ server (JSON hoặc text)
        ...
        try {
          ...
          if (ct.includes('application/json')) {
            ...
            errMsg = (await zipRes.text()).slice(0, 120) || errMsg;
          }
        } catch (_) {}
      ...
      showToast('✅ Tải xuống ZIP thành công (' + total + ' file)!', 'ok');
    } catch (e) {
