...
// Đảm bảo thư mục tồn tại (backup_storage đã được tạo ở phần BACKUP MODULE)
// Hàm loadPersistedDevices / savePersistedDevices gọi sau khi thư mục sẵn sàng.
...
function savePersistedDevices() {
  try {
    ...
    fs.writeFileSync(DEVICES_PERSIST_FILE, JSON.stringify(arr, null, 2), 'utf8');
  } catch (e) {
...
function loadPersistedDevices() {
  try {
    if (!fs.existsSync(DEVICES_PERSIST_FILE)) return;
    const arr = JSON.parse(fs.readFileSync(DEVICES_PERSIST_FILE, 'utf8'));
    for (const d of arr) {
